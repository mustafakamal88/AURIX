...
class BrokerReconciliationStore:
    ...
    def _write_status(self, latest: dict[str, Any] | None) -> dict[str, Any]:
        ...
        status = {
            ...
            "broker_position_count": latest.get("positions_count", len(latest.get("broker_positions") or [])) if latest else 0,
            "broker_order_count": latest.get("orders_count", len(latest.get("broker_orders") or [])) if latest else 0,
            "mismatch_count": latest.get("mismatches_count", len(latest.get("mismatches") or [])) if latest else 0,
            "warning_count": len(latest.get("warnings") or []) if latest else 0,
            "unexpected_exposure": bool(latest.get("unexpected_exposure")) if latest else False,
            "dirty_evidence_found": bool(latest.get("dirty_evidence_found")) if latest else False,
            "reasons": latest.get("reasons") if latest else [],
