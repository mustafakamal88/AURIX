...
def main() -> int:
    ...
    interval = float(os.getenv("AURIX_XAUUSD_PAPER_V2_WATCH_SECONDS", "5"))
    base_url = f"http://{host}:{port}"
    print(f"Watching XAUUSD Paper V2 every {interval:g}s. Press Ctrl+C to stop.")
    try:
        while True:
            try:
                context = post_json(f"{base_url}/context/evaluate")
                paper_result = post_json(f"{base_url}/paper/evaluate-paper-v2")
