...
from aurix_strategy_engine.xauusd_paper_v2 import XauusdPaperV2Config
...
class XauusdPaperV2BacktestReplayEngine:
    def __init__(self, config: XauusdPaperV2Config):
        self.config = config
    ...
    def run(self, candles: list[dict[str, Any]]) -> tuple[BacktestReport, list[BacktestTrade]]:
        ...
        warnings = [
            "v2 backtest is paper/research only",
            "sample size may be low; do not infer profitability without larger forward evidence",
        ]
        trades: list[BacktestTrade] = []
        signals = 0
        if not sorted_candles:
        ...
        if len(sorted_candles) < self.config.min_candles:
            warnings.append("insufficient candles")
        if not self.config.enabled:
            warnings.append("strategy disabled")

        index = max(self.config.min_candles, self.config.lookback_range_candles + 1)
        session_signal_counts: dict[str, int] = {}
        while self.config.enabled and index < len(sorted_candles):
            signal = self._signal_at(sorted_candles, index, session_signal_counts)
            if signal is None:
                index += 1
                continue
        ...
        return self._report(len(sorted_candles), signals, trades, warnings), trades

    def _signal_at(self, candles: list[dict[str, Any]], index: int, session_signal_counts: dict[str, int]) -> Optional[dict[str, Any]]:
        previous = candles[index - 1]
        current = candles[index]
        session = str(current.get("session_name") or current.get("session") or "LONDON")
        regime = str(current.get("regime") or "RANGE")
        if self.config.block_closed_session and session == "CLOSED":
            return None
        if session not in self.config.allowed_sessions:
            return None
        if session_signal_counts.get(session, 0) >= self.config.max_signals_per_session:
            return None
        if regime in {"HIGH_SPREAD", "INSUFFICIENT_DATA"}:
            return None
        if self.config.avoid_chop and regime == "CHOP":
            return None
        if not self._spread_ok(current):
            return None
        ...
        if len(lookback) < self.config.lookback_range_candles:
            return None
        highs = [_as_float(candle.get("high")) for candle in lookback]
        lows = [_as_float(candle.get("low")) for candle in lookback]
        ...
        current_close = _as_float(current.get("close"))
        if None in {previous_low, previous_high, previous_close, current_open, current_high, current_low, current_close}:
            return None
        candle_range = max(float(current_high) - float(current_low), 0.0)
        body = abs(float(current_close) - float(current_open))
        meaningful_body = candle_range > 0 and body / candle_range >= 0.45

        if (
            self.config.allow_buy
            and float(previous_low) < range_low
            and float(current_close) > range_low
            and float(current_close) > float(current_open)
            and meaningful_body
            and float(current_close) > float(previous_close)
        ):
            return {"direction": "BUY", "range_low": range_low, "range_high": range_high, "session": session, "reasons": ["v2 bullish sweep reclaim"]}
        if (
            self.config.allow_sell
            and float(previous_high) > range_high
            and float(current_close) < range_high
            and float(current_close) < float(current_open)
            and meaningful_body
            and float(current_close) < float(previous_close)
        ):
            return {"direction": "SELL", "range_low": range_low, "range_high": range_high, "session": session, "reasons": ["v2 bearish sweep reclaim"]}
        return None
    ...
    def _simulate_trade(self, candles: list[dict[str, Any]], entry_index: int, signal: dict[str, Any]) -> tuple[BacktestTrade, int]:
        ...
        trade = BacktestTrade(
            ...
            stop_loss=stop_loss,
            take_profit=take_profit,
            setup_name="xauusd_paper_v2_replay",
            reasons=signal["reasons"],
        )
        for index in range(entry_index + 1, len(candles)):
            candle = candles[index]
